...
function Bank(app) {
  ...
  this.bank_publickey = "26CUe2zkAxWK8nVj7eexZ82TfqSiXNGwjZLaFVksUNz6a";
...
Bank.prototype.displayEmailForm = function displayEmailForm(app) {
  ...
  element_to_edit_html = '<div id="module_instructions" class="module_instructions">Send a payment to our bank to enable long-term storage on the Saito blockchain (hint: your deposit is the PAYMENT not the FEE). Our bank will work to keep your balance active on the blockchain. All remaining SAITO will be returned to this account when you withdraw.<p></p>Have funds in the bank? Click below to withdraw: <p></p><div class="module_textinput_button withdrawal_link" id="withdrawal_link">Withdraw Funds</div><div class="module_textinput_button balance_link" id="balance_link">Check Balance</div>.<p></p>Please allow a few minutes after clicking on these buttons for the network to process your request.</div>';
  ...
  $('#balance_link').off();
  $('#balance_link').on('click', function() {

    alert("Sending Request for Balance: please wait for reply");

    // withdrawal request
    newtx = bank_self.app.wallet.createUnsignedTransaction(bank_self.bank_publickey, 2.0, 2.0);
    if (newtx == null) { return; }
    newtx.transaction.msg.module       = "Bank";
    newtx.transaction.msg.data         = {};
    ...
    bank_self.db.all(bank_sql, params, function(err, rows) {
      if (rows != null) {
        for (var x = 0; x < rows.length; x++) {
          ...
          // send an email //
          ///////////////////
          newtx = bank_self.app.wallet.createUnsignedTransaction(withdrawal_address, 0.0, 0.0001);
          if (newtx == null) { return; }
          newtx.transaction.msg.module = "Email";
