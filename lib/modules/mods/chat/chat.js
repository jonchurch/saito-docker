...
console.log(txmsg.message);

console.log("USING DECRYPTION KEYS FOR: " + txmsg.from);

  // force decryption if key exists
  // no sanity check
  var message = app.keys.decryptString(txmsg.from, txmsg.message);
  var sig     = txmsg.sig;
  var from    = txmsg.from;
  var to      = txmsg.to;
  var sig     = txmsg.sig;
...
console.log("PUSHING: " + JSON.stringify(newmsg));
