...
/*
    ...
    if (txmsg.relay == 1) {
      ...
      // 
      // not for us, see if we can fwd
      //
      let rhost = txmsg.relay_host;
      let rport = txmsg.relay_port;
      let rpubkey = txmsg.relay_pubkey;

      let peers = chat_self.app.network.peers;

      for (let p = 0; p < peers.length; p++) {
        if (peers[i].peer.host == rhost && peers[i].peer.port == rhost && peers[i].peer.publickey == rhost) {

console.log("Hey! We are able to relay this to someone else!");

          relreq = {};
          relreq.request = "chat send message";
          relreq.data = {};
          relreq.data.tx = req.data.tx;
          relreq.data.counter = 1;
          peers[i].sendRequest(newreq.request, newreq.data);
...
console.log("ADDING: " + pubkey_to_add);
	...
	return;
      ...
      //
      let dest_addy   = chat_self.chat.rooms[chatRoom].publickey;
      let dest_port   = chat_self.chat.rooms[chatRoom].publickey;
      let dest_host   = chat_self.chat.rooms[chatRoom].publickey;
      let dest_pubkey = chat_self.chat.rooms[chatRoom].publickey;
      if (addy == null) { addy = chat_self.app.wallet.returnPublicKey(); }
