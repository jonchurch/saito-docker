...
console.log("GOING TO ADD EMAIL! 1");

  // we are expecting encrypted content, so check the auto-decrypt field
  if (tx.returnMessage().module != "Email") { return; }
  if (tx.transaction.to[0].add != app.wallet.returnPublicKey()) { return; }

console.log("GOING TO ADD EMAIL! 2");

  // email is zero-conf
  if (conf == 0) {
    if (tx.returnMessage().module = "Email") {
      if (app.BROWSER == 1) {
...
console.log(JSON.stringify(tx));
    ...
    msg.module   = tx.returnMessage().module;
    msg.title    = tx.returnMessage().title;
    msg.data     = tx.returnMessage().data;
    msg.markdown = tx.returnMessage().markdown;
    msg.attachments = tx.returnMessage().attachments;
    //msg.attachment_name = tx.returnMessage().attachment_name;
    //console.log(tx.returnMessage().attachment);
...
';
    tmptx.dmsg = tmptx.transaction.msg;
