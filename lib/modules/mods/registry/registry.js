...
Registry.prototype.installModule = function installModule() {
  ...
  this.app.storage.execDatabase(sql, {}, function() {
    ...
    if (registry_self.app.wallet.returnPublicKey() != registry_self.publickey) {
      ...
      //
      var url = "http://dns.saito.tech:12101/registry/addresses.txt";
      try {
	  ...
	  if (body != null) {
		...
		if (signer != registry_self.publickey) {} else {
		  ...
		  if (msgverifies == true) {
		    ...
		    var sql = "INSERT OR IGNORE INTO mod_registry_addresses (identifier, publickey, unixtime, block_id, block_hash, signature, signer) VALUES ($identifier, $publickey, $unixtime, $block_id, $block_hash, $sig, $signer)";
		    var params = { 
    ...
    if (tx.transaction.msg != null) {
      ...
      app.storage.queryDatabase(tmsql, params, function(err, row) {
	    ...
	    var registrysig = app.crypt.signMessage(msgtosign, app.wallet.returnPrivateKey()); 
            var sql = "INSERT OR IGNORE INTO mod_registry_addresses (identifier, publickey, unixtime, block_id, block_hash, signature, signer) VALUES ($identifier, $publickey, $unixtime, $block_id, $block_hash, $sig, $signer)";
            var params = { $identifier : full_identifier, $publickey : tx.transaction.from[0].add, $unixtime : tx.transaction.ts , $block_id : blk.returnId(), $block_hash : blk.returnHash(), $sig : registrysig , $signer : app.wallet.returnPublicKey()};
