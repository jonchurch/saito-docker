  ...
  if (conf == 0) {
    ...
    if (tx.transaction.msg != null) {
      ...
      if (txmsg.module == "Email") {
        ...
        if (tx.transaction.to[0].from != registry_self.publickey) { return; }
        if (txmsg.sig == "") { return; }
  ...
  registry_self.db.get(tmsql, params, function(err, row) {
    if (row != null) {
      if (row.count == 0) {
        ...
        var sqlwrite = full_identifier + "\t" + blk.block.id + "\t" + blk.returnHash() + "\t" + tx.transaction.from[0].add + "\t" + tx.transaction.ts + "\t" + registrysig + "\t" + registry_self.app.wallet.returnPublicKey() + "\n";
