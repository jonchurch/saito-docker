...
AppStore.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/appstore/cache/:ffolder/:ffile', function (req, res) {
    if (req.params.ffolder.indexOf("\/") != false) { return; }
    if (req.params.ffile.indexOf("\/")   != false) { return; }
    var ffolder = '/web/cache/'+req.params.ffolder;
    var ffile   = '/web/cache/'+ffolder+'/'+req.params.ffile;
    res.sendFile(__dirname + ffile);
    return;
  });
  ...
  expressapp.get('/appstore/status', function (req, res) {
    ...
    if (appstore_self.currently_generating == 1 && appstore_self.unique_no == packagename) {
      ...
      results.status           = 1;
      results.browser_js       = "/appstore/cache/" + packagename + "/browser.js";
      results.chrome_extension = "/appstore/cache/" + packagename + "/chrome.crx";
    }
...
console.log("KICKING TO HANDLE ZIP FILES");
...
console.log("HERE WE ARE: " + createcmp4);
  ...
  if (shell.exec(mkdircmd).code !== 0) {
    ...
    if (shell.exec(createcmp5).code !== 0) {
      ...
      if (shell.exec(createcmp6).code !== 0) {
        ...
        let chromecmd = "../../../../node_modules/.bin/crx -o web/cache/"+appstore_self.unique_no+"/chrome.crx pack ../../../../extras/chrome";
        if (shell.exec(createcmp6).code !== 0) {
          appstore_self.currently_generating = 0;
...
<body> \
 ...
 \
  <div class="main" style="display:none"> \
    ...
    <p></p> \
    <a href="" id="link_browserjs">Download BrowserJS (developers)</a> \
    <br /> \
    <a href="" id="link_chrome">Download chrome extension</a> \
    <p></p> \
