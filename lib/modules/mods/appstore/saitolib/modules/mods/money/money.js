...
Money.prototype.returnTableContents = function returnTableContents(app) {
  ...
  for (var b = mv, bind = 0; b >= ma; b--, bind++) {
    ...
    blockid[bind]    = b;
    treasury[bind]   = parseFloat(tmpblk.returnTreasury())-parseFloat(tmpblk.returnCoinbase());
    ...
    if (tmpblk != null) {
      ...
      for (var bt = 0; bt < tmpblk.transactions.length; bt++) {
        ...
        if (tmpblk.transactions[bt].isGoldenTicket() == 1) { 
	  gt[bind] = 1; 
        }
...
console.log(unspent[bind] + " += " + slip1.amt);
	    unspent[bind] = parseFloat(unspent[bind]) + parseFloat(slip1.amt);
console.log("RESULT: " + unspent[bind]);
	  }
        }
      }
    }
...
console.log("TOTAL UNSPENT: " + total_unspent + " + " + unspent[sd]);
    total_unspent   = parseFloat(total_unspent) + parseFloat(unspent[sd]);
console.log("TOTAL UNSPENT: " + total_unspent);
    ...
    bdata += '<tr> \
		...
		<td>'+gt[sd]+'</td> \
		<td>'+reclaimed[sd]+'</td> \
		<td>'+parseFloat(spent[sd])+'</td> \
		<td>'+parseFloat(unspent[sd])+'</td> \
		<td>'+fees[sd]+'</td> \
	      </tr>';	
  }
  ...
  bdata += '<tr> \
		...
		<td>'+parseFloat(total_unspent)+'</td> \
		<td></td> \
		<td>'+(parseFloat(tr)+parseFloat(total_coinbase)+parseFloat(total_reclaimed)+parseFloat(total_unspent)).toFixed(8)+'</td> \
	      </tr>';	
