...
Advert.prototype.fetchAdvert = function fetchAdvert(width, height) {
  ...
  $.getJSON('/advert/'+this.app.wallet.returnPublicKey()+'/'+width+'/'+height, function (data) {
    if (data.id != null) {
      htmlToInsert = '<a href="'+data.link+'"><img style="border:1px solid #39579a;width:'+width+'px;height:'+height+'px" src="/advert/cache/'+data.adfile+'" /></a>';
    } else {
...
console.log(psql + " -- " + JSON.stringify(pparams));
