...
Peer.prototype.addSocketEvents = function addSocketEvents() {
  ...
  this.socket.rn = Math.floor(Math.random()*10000000);
  ...
  try {
    ...
    this.socket.on('connect', function(){
      console.log("client connect: " + peer_self.socket.rn);
      this.initializing = 0;
...
console.log("INCOMING req from " + peer_self.socket.rn + ": " + message.request);
	...
	var tmpblk = peer_self.app.blockchain.returnBlockByHash(lasthash);
      ...
      if (message.request == "block") {
        ...
        if (peer_self.peer.synctype == "full") {
          ...
          peer_self.app.blockchain.importBlock(message.data, expected_block_hash);
	  ...
	  // it here.
	  // 
          peer_self.app.blockchain.importBlock(message.data, expected_block_hash, 0);
        }
        return;
      }
...
console.log("remote server initiated connection request");
console.log("sending them something in return.... CRM");
console.log(this.returnConnectResponseMessage());
...
Peer.prototype.sendBlock = function sendBlock(message, blk, instant=1) {
  ...
  if (instant == 1) {
    if (this.socket != null) {
      if (this.socket.connected == true) {
console.log("OUTGOING req through " + peer_self.socket.rn);
...
console.log("OUTQUEUE req through " + peer_self.socket.rn);
