...
Blockchain.prototype.validateBlockAndQueueInMempool = function validateBlockAndQueueInMempool(blk, relay_on_validate=1) {
  ...
  if (! blk.validate() ) {
    ...
    this.app.logger.logError(`INVALID BLOCK HASH: ${blk.returnHash()}`,
      {message:"",err:""});
    blk.block.transactions = [];
    blk.transactions = [];

    console.log(JSON.stringify(blk.block, null, 4));
    this.app.logger.logError(JSON.stringify(blk.block, null, 4),
      {message:"",err:""});
    return 0;
  }
  ...
  blk.validateReclaimedFunds(function(validated_or_not) {
    ...
    if (validated_or_not == 0) {
      ...
      this.app.logger.logError("Reclaimed Funds found invalid",
        {message:"",err:""});
...
Blockchain.prototype.addBlockToBlockchain = function addBlockToBlockchain(newblock, forceAdd="no") {
  ...
  this.app.logger.logInfo(`Adding block ${block_id} -> ${hash} ${newblock.block.unixtime}`)
...
console.log("\n\n\nIN BLOCKCHAIN RESETTING SPENT INPUTS\n\n");
      ...
      for (let i = 0; i < our_longest_chain.length && i < blockchain_self.callback_limit; i++) {
        ...
        if (thisblk != null) {
          ...
          try {
            ...
            this.app.logger.logError("Error thrown in addBlocktoBlockchainPart2", err);
          }
