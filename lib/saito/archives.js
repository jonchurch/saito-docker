...
console.log("Testing Saving Remotely 1!");
...
console.log("Testing Saving Remotely 2!");

    if (this.archives[aas].active == "active") {
      
console.log("Testing Saving Remotely 3!");
      ...
      var message                      = {};
          ...
          message.data.txsig           = tx.transaction.sig;
...
console.log("HOW MANY PEERS DO WE HAVE? " + this.app.network.peers.length);

      for (let y = 0; y < this.app.network.peers.length; y++) {

console.log(this.app.network.peers[y].peer.publickey + " -- " + this.archives[aas].publickey);

        if (this.app.network.peers[y].peer.publickey = this.archives[aas].publickey) {
console.log("SENDING TX TO ARCHIVE");
	  this.app.network.peers[y].sendRequest(message.request, message.data);
        }
      }
    }
