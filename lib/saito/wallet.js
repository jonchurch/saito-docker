...
console.log("total_fees " + " -- " + this.returnBalance() + " -- " + total_fees);

  if (total_fees > this.returnBalance()) { return null; }

  //
  // testing -- create fake input if total == 0, as it will not need to be confirmed
  //
  if (total_fees == 0.0) {
    tx.transaction.from = [];
...
console.log("tx is from: " + JSON.stringify(tx.transaction.from));
...
console.log("returning a valid tx..." + JSON.stringify(tx));
...
console.log(this.spent_slips_idx + " -- " + this.wallet.utxi.length);

  for (let i = this.spent_slips_idx; i < this.wallet.utxi.length; i++) {
console.log("i: " + i + " ----> " + this.wallet.utxi.length);
