...
function Wallet(app) {
  ...
  // module.
  //
  this.store_utxo                 = 1; // 1 = store utxo
...
Wallet.prototype.containsUtxi = function containsUtxi(s) {
  ...
  // check on non-longest chain
  s.lc = 0;
  hmi = this.returnHashmapIndex(s);
  if (this.utxi_hashmap[hmi] == 1) { 
    s.lc = orig;
    return 1; 
  }
...
Wallet.prototype.containsUtxo = function containsUtxo(s) {
  ...
  s.lc = 1;
  let hmi = this.returnHashmapIndex(s);
  if (this.utxo_hashmap[hmi] == 1) { s.lc = orig; return 1; }
