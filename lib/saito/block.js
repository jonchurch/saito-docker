  ...
  if (eliminated_block < 1) {
    ...
    block_self.app.storage.queryDatabase(sql, params, function(err, row) {
      ...
      block_self.app.storage.openBlockByFilename(filename, function(storage_self, blk) {
        ...
        if (row == null) {
          console.log("Error opening block that is supposed to be on disk: " + filename);
          process.exit(0);
        }   
...
// Validation //
////////////////
Block.prototype.validate = function validate(mycallback=null) {

  var block_self = this;

  if (mycallback == null) {
    console.log("validating block without callback!");
    process.exit();
  }
  ...
  //
  //if (block_self.validateGoldenTicket() == 0 && block_self.app.SPVMODE == 0) {
  //  console.log("Block invalid: contains invalid golden ticket");
  //  block_self.app.mempool.removeGoldenTicket();
  //  mycallback(0, null);
  //  return;
  //}
  ...
  for (var zz = 0; zz < block_self.transactions.length; zz++) {
    if (block_self.transactions[zz].validate(block_self.app, block_self.block.paysplit_vote, block_self.block.id) != 1) {
...
console.log("validating hash: " + block_self.returnHash());
console.log("About to try and fetch previous block: " + block_self.block.prevhash);

  ///////////////////////////
  // burn fee and fee step //
  ///////////////////////////
  if (block_self.block.prevhash != "") {
    block_self.app.blockchain.returnBlockByHash(block_self.block.prevhash, function (prevblk) {
      if (prevblk != null) {
console.log("PREVBLK NOT NULL");
        var newbf = block_self.calculateBurnFee(prevblk.returnBurnFee(), prevblk.returnFeeStep());
        if (newbf[0] != block_self.block.burn_fee) {
          console.log("Block invalid: burn fee miscalculated: "+newbf[0]+" versus "+block_self.block.burn_fee);
          mycallback(0, null);
	  return;
        }
        if (newbf[1] != block_self.block.fee_step) {
          console.log("Block invalid: fee step miscalculated: "+newbf[1]+" versus "+block_self.block.fee_step);
          mycallback(0, null);
	  return;
        }
      }
console.log("RETURNING PREVBLK");
      mycallback(1, prevblk);
    });
  } else {
    if (block_self.app.blockchain.blocks.length <= 1) {
      mycallback(1, null);
    } else {
      mycallback(0, null);
    }
