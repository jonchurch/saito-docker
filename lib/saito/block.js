  ...
  if (eliminated_block < 1) {
    ...
    block_self.app.storage.queryDatabase(sql, params, function(err, row) {
      ...
      if (row == null) {
        ...
        this.app.logger.logError("Error handling monetary policy....",
            {message:"",err:""});
        process.exit(0);
      }
            ...
            block_self.app.storage.queryDatabase(sql3, params3, function(err3, row3) {
              ...
              block_self.app.storage.openBlockByFilename(filename3, function(storage_self3, blk3) {
                ...
                if (blk3 == null) {
                  ...
                  this.app.logger.logError(`Error handling monetary policy: block missing from disk: ${filename3}`,
                    {message:"",err:""});
                  process.exit(0);
                }
...
Block.prototype.validate = function validate() {
  ...
  for (var zz = 0; zz < block_self.transactions.length; zz++) {
    if (block_self.transactions[zz].validate(block_self.app, block_self.block.paysplit_vote, block_self.block.id) != 1) {
      ...
      this.app.logger.logError("Block invalid: contains invalid transaction", {message:"",err:""});
      this.app.logger.logError(`hash: ${block_self.app.crypt.hash(JSON.stringify(block_self.transactions[zz]))}`);
      this.app.logger.logError(`sig: ${block_self.transactions[zz].transaction.sig}`);
      this.app.logger.logError(`msig: ${block_self.transactions[zz].transaction.msig}`);
      return 0;
    }
