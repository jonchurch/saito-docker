...
function Peer(app) {
  ...
  //
  // this makes sure that messages will eventually be
  // delivered even if the browser/server loses its
  // connection briefly.
  //
  var peer_self = this;
...
Peer.prototype.addSocketEvents = function addSocketEvents() {
  ...
  try {
	...
	let lasthash = message.data.lasthash;
		
	// check to see what the ID of the last hash is....
	var tmpblk = peer_self.app.blockchain.returnBlockByHash(lasthash);



        if (peer_self.app.BROWSER == 0 && peer_self.app.SPVMODE == 0) {
          peer_self.app.storage.openBlockByHash(t.hash, function(storage_self, blk) {
	    ...
	    if (blk.returnId() > lastblkid) {
	      ...
	      peer_self.sendBlock("block", blk, 1) 
	    }
      ...
      // TODO
      //
      // THIS IS NOT IMPLEMENTED YET -- NOTIFY OF BLOCK AVAILALBLE
      // 
      // AND LET USER DECIDE IF WORTHWHILE
      // 
      /////////////////////
      // block available //
      /////////////////////
      if (message.request == "block available") {
console.log("WE HAVE BEEN TOLD A BLOCK IS AVAILABLE: ");
console.log(JSON.stringify(message));
        if (message.data == null) { return; }
        if (message.data.block_hash == null) { return; }
        let block_hash     = message.data.block_hash;
...
console.log("RECEIVED TX: " + JSON.stringify(message));
...
Peer.prototype.connect = function connect(remote = 0) {
  ...
  if (remote == 0) {
    ...
    if (this.app.options.server != null) {
      if ((this.peer.post == this.app.options.server.port && this.peer.host == this.app.options.server.host) && this.app.BROWSER == 0) { 
        return; 
      }
    }
...
console.log("SENDING: " + this.app.storage.currently_reindexing);
...
Peer.prototype.sendRequest = function sendRequest(message, data="", instant=0) {
  ...
  // never connect.
  //
  if (instant == 1 && this.initializing == 0) {
    if (this.socket != null) {
      if (this.socket.connected == true) {
console.log("INSTANT SEND: " + JSON.stringify(userMessage));
        this.socket.emit('request',JSON.stringift(userMessage));
      }
    }
  } else {
console.log("QUEUED SEND: " + JSON.stringify(userMessage));
    this.message_queue.push(JSON.stringify(userMessage));
    if (this.initializing == 1) {
      return;
    }
    if (this.socket != null) {
      if (this.socket.connected != true) {
