...
Peer.prototype.sendBlock = function sendBlock(message, blk, instant=0) {
  ...
  if (this.peer.synctype == "lite") {
    ...
    // TODO
    //
    // if we have the filename written down, we can 
    // send an alert that we have the block and let
    // the remote client fetch it from us if they 
    // want.
    //
    if (blk.filename == "") {
  ...
  if (instant == 1) {
    ...
    if (this.socket != null) {
      if (this.socket.connected != true) {
console.log("QUEUE CLEANUP 1");
        this.app.network.cleanupDisconnectedSocket(this);
        return;
      }
    } else {
console.log("QUEUE CLEANUP 2");
      this.app.network.cleanupDisconnectedSocket(this);
      return;
    }
  }
...
Peer.prototype.sendRequest = function sendRequest(message, data="", instant=0) {
  ...
  if (instant == 1 && this.initializing == 0) {
    ...
    if (this.socket != null) {
      if (this.socket.connected != true) {
console.log("QUEUE CLEANUP 3");
        this.app.network.cleanupDisconnectedSocket(this);
        return;
      }
    } else {
console.log("QUEUE CLEANUP 4");
      this.app.network.cleanupDisconnectedSocket(this);
      return;
    }
  }
