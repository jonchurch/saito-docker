  ...
  try {
    ...
    this.socket.on('connect', function(){
      ...
      if (peer_self.contact == 0) {
        if (peer_self.disconnected == 1) {
          let message = peer_self.returnConnect();
	...
	////////////////
      ...
      if (message.request == "missing block") {
        let tmpr = JSON.parse(message.data);
      ...
      if (message.request == "block available") {
        ...
        peer_self.app.blockchain.mempool.fetchBlock(peer_self, block_hash);
        return;
      }
...
// node in the network. If we are initiating the connection
// then we set our socket events.
//
// In either case, we format a JS message object and send 
// it to the remote server so that we can handle mutual 
// syncing, etc.
//
...
Peer.prototype.connect = function connect(remote = 0) {
  ...
  if (remote == 0) {
    ...
    if ( 
      (
        this.peer.post == this.app.options.server.port && 
	this.peer.host == this.app.options.server.host 
      ) && 
      this.app.BROWSER == 0
    ) { return; }
    ...
    //
    this.socket.emit('request',JSON.stringify(returnConnectResponseMessage()));
