...
Storage.prototype.execDatabase = function execDatabase(sql, params, callback=null) {
  if (this.app.BROWSER == 1) { return; }
  this.db.run(sql, params, function (err, row) { if (callback != null) { callback(err, row); } });
}
...
console.log(peer.sync_sending_bid + " -- " + peer.sync_latest_bid + " -- " + peer.sync_sending_db_bid + " -- " + peer.sync_sending);
