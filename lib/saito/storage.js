...
Storage.prototype.execDatabase = function execDatabase(sql, params, callback=null) {
  ...
  this.db.run(sql, params, function (err, row) {
    ...
    if (err) {
      this.app.logger.logError("Error thrown in execDatabase", err);
    }
  });
}
      ...
      try {
        storage_self.openBlockByFilename(files[current_block], function(storage_self, blk) {
          ...
          if (blk == null) {
            ...
            this.app.logger.logError(`Error loading block from disk: missing block: ${files[current_block]}`,
              { message: "", error: "" });
            process.exit();
          }
          ...
          // LOGGING INFO
          this.app.logger.logInfo(`REPOPULATING: adding block to mempool w/ id: ${blk.returnId()} -- ${blk.returnHash()}`)
    ...
    if (data == null) {
      $.ajax({
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
          this.app.logger.logError("Reading client.options from server failed", errorThrown)
        }
      });
    }
  }
...
Storage.prototype.onChainReorganization = function onChainReorganization(block_id, block_hash, lc) {
  ...
  this.db.run(sql, {
    ...
    this.app.logger.logError("db.run in onChainReorganization failed", err)
  });
}
...
Storage.prototype.openBlockByFilename = function openBlockByFilename(filename, mycallback) {
  ...
  try {
    ...
    this.app.logger.logError("Error reading block from disk", err)
  }
}
...
Storage.prototype.queryDatabase   = function queryDatabase(sql, params, callback) {
  ...
  this.db.get(sql, params, function (err, row) {
    ...
    if (err) {
      this.app.logger.logError("Error from queryDatabase", err)
    }
  });
}
...
Storage.prototype.queryDatabaseArray = function queryDatabaseArray(sql, params, callback) {
  ...
  this.db.all(sql, params, function (err, rows) {
    ...
    if (err) {
      this.app.logger.logError("Error from queryDatabaseArray", err);
    }
  });
}
...
Storage.prototype.saveBlock = function saveBlock(blk, lc = 0) {
  ...
  storage_self.db.run(sql2, params2, function(err) {
    ...
    if (err) {
      this.app.logger.logError("Error thrown in storage.saveBlock", err);
    }
...
Storage.prototype.saveOptions = function saveOptions() {
  ...
  if (this.app.BROWSER == 0) {
    fs.writeFileSync("options", JSON.stringify(storage_self.app.options), function(err) {
      if (err) {
        this.app.logger.logError("Error thrown in storage.saveOptions", err);
        console.log(err);
        return;
      }
    });
  ...
  this.app.storage.queryDatabaseArray(sql, params, function (err, rows) {
    ...
    for (let r = 0; r < rows.length; r++) {
      ...
      try {
        ...
        this.app.logger.logError("Error reading block from disk", {message: "", error: ""});
        this.app.logger.logError("Error Reading Block File", {message: "", error: ""});
      }
    }
    if (peer.isConnected() != 1) {
      peer.sync_sending = 0;
...
Storage.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, current_unwind_index, resetting_flag) {
  ...
  this.app.logger.logInfo(" ... unwind old chain");
  ...
  if (old_block_hashes.length > 0) {
    ...
    storage_self.app.blockchain.returnBlockByHash(old_block_hashes[current_unwind_index], function(blk) {
      ...
      if (blk == null) {
        ...
        this.app.logger.logError("We cannot find a block on disk that should exist in unwindChain",
          { message: "", err: "" });
	      process.exit();
      }
...
//
Storage.prototype.validateBlockInputs = function validateBlockInputs(newblock, prevblk) {

  console.log(" ... validate block inputs and other stuff");
  this.app.logger.logInfo(" ... validate block inputs and other stuff");
  ...
  for (var ti = 0; ti < newblock.transactions.length; ti++) {
    ...
    for (var ti2 = 0; ti2 < tx.transaction.from.length; ti2++) {
      ...
      if (tmpft == 1) {
        if (tmpftfound == 1) {
          // LOG INFO
          this.app.logger.logError("Block invalid: multiple fee capture transactions in block",
            {message: "", err: ""});
      ...
      if (newblock.transactions[b].transaction.from[bb].amt > 0) {
        ...
        if (shashmap.validate_slip(slip_map_index, newblock.block.id) == 0) {
          ...
          if (newblock.transactions[b].transaction.from[bb].bid < storage_self.app.blockchain.blk_limit) {
            ...
            // LOG INFO
            this.app.logger.logError("Validation Failure, but acceptable as we do not have a full genesis period yet",
              {message: "", err: ""});
            ...
            this.app.logger.logInfo(`FAILED TO VALIDATE SLIP: ${slip_map_index}`);
            this.app.logger.logInfo(JSON.stringify(newblock.transactions[b].transaction.from[bb], null, 4));
            this.app.logger.logInfo(`MY GBID: ${storage_self.app.blockchain.blk_limit}`);
  ...
  //
  if (prevblk == null) {
    if (storage_self.app.blockchain.index.length > 0) {
      return 0;
    } else {
      return 1;
    }
    ...
    if (! newblock.validateGoldenTicket(prevblk) ) {
      ...
      this.app.logger.logError("Block does not validate -- Golden Ticket Wrong!!!",
        {message:"", error:""});
      return 0;
    }
    ...
    if (newbf[0] != newblock.block.burn_fee) {
      ...
      this.app.logger.logError("Block invalid: burn fee miscalculated: "+newbf[0]+" versus "+newblock.block.burn_fee,
        {message:"", error:""});
      return;
    }
    if (newbf[1] != newblock.block.fee_step) {
      ...
      this.app.logger.logError("Block invalid: fee step miscalculated: "+newbf[1]+" versus "+newblock.block.fee_step,
        {message:"", error:""});
      return 0;
    }
    ...
    storage_self.app.blockchain.returnBlockByHash(new_block_hashes[current_wind_index], function(blk) {
      storage_self.app.blockchain.returnBlockByHash(blk.block.prevhash, function(prevblk) {
        ...
        if (blk == null) {
          ...
          this.app.logger.logError("Cannot open block that should exist in windChain",
            { message: "", error: "" });
          process.exit();
        }
